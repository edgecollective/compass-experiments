...
void display_map(float lat, float lon, uint8_t max_x, uint8_t max_y, uint8_t x, uint8_t y, char locator[]) {
  ...
  //minor frame
  display.drawLine(0,round(max_y/2),max_x,round(max_y/2),COLOR1);
  display.drawLine(round(max_x/2),0,round(max_x/2),max_y,COLOR1);


  display.drawCircle(x, y, 6, COLOR1);
  display.fillCircle(x, y, 3, COLOR1);

  // info display
  //display.drawLine(90,max_y,90,150,COLOR1);
