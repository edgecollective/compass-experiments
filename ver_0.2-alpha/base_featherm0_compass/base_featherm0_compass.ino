...
if (manager->recvfromAckTimeout((uint8_t *)buf, &len, waitTime, &from)) {  // this runs until we receive some message
  ...
  float degree_diff = brng-heading;
  ...
  Serial.println(degree_diff);
   ...
   int dy = round(r*sin((theta_graph-90)*PI/180.));
   int dx = round(r*cos((theta_graph-90)*PI/180. ));
   ...
   int cdx = round(r*cos((heading-90)*PI/180.));
   int cdy = round(r*sin((heading-90)*PI/180.));
